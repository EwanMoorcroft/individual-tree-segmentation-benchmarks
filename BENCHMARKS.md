...
| for-instance | tls2trees | tls2trees_for_instance_published_default_leaf_off | external_training_only | class3_ignore_leaf_off_test | Frozen full-test workflow ready; result pending separate Barkla execution | [`published-default test config`](methods/tls2trees/configs/for_instance_published_default_test.yml); [`runbook`](methods/tls2trees/docs/for_instance_published_default_smoke.md) |
...
| for-instance | tls2trees | tls2trees_for_instance_published_default_leaf_on | external_training_only | class3_ignore_leaf_on_test | Frozen full-test workflow ready; result pending separate Barkla execution | [`published-default test config`](methods/tls2trees/configs/for_instance_published_default_test.yml); [`runbook`](methods/tls2trees/docs/for_instance_published_default_smoke.md) |
...
| for-instance | tls2trees | tls2trees_for-instance_development_tuned_leaf_screen_20260720_193825 | external_training_only | development_leaf_attachment_screen | Completed development-only diagnostic; 45/45 valid metrics and identical aggregate accuracy across nine settings; public-safe export ready from retained evidence | [`screen config`](methods/tls2trees/configs/for_instance_development_tuned_leaf_screen.yml); [`publication finaliser`](methods/tls2trees/scripts/evaluation/finalise_tls2trees_development_leaf_screen.py) |
...
shared pointwise protocol and one completed TLS2trees development-tuned row
using the explicit class-3-ignore scoring domain. All use the supplied 11-plot
...
TLS2trees leaf-off target remain in the diagnostics table. TLS2trees retains
both target prediction sets as 22 hash-verified source-row files.
