...
TOOLCHAIN_LOCK="$METHOD_ROOT/locks/conda_toolchain_explicit_linux-64.txt"
...
TOOLCHAIN_LOCK_SHA256="$(sha256sum "$TOOLCHAIN_LOCK" | cut -d ' ' -f 1)"
BUILD_INPUT_SHA256="$(
  {
...
BUILD_JOB="$(
  sbatch --parsable \
    ...
    --export=ALL,FF3D_BUILD_CONFIRMED=1,FF3D_BENCHMARK_ROOT="$BENCHMARK_ROOT",FF3D_BENCHMARK_COMMIT="$BENCHMARK_COMMIT",FF3D_ROOTLESS_BUILDER="$ROOTLESS_BUILDER",FF3D_ROOTLESS_BUILDER_SHA256="$ROOTLESS_BUILDER_SHA256",FF3D_TOOLCHAIN_LOCK_SHA256="$TOOLCHAIN_LOCK_SHA256",FF3D_BUILD_INPUT_SHA256="$BUILD_INPUT_SHA256",FF3D_BASE_SIF="$BASE_SIF",FF3D_BASE_SIF_SHA256="$BASE_SIF_SHA256",FF3D_ENV_ROOT="$ENV_ROOT",FF3D_RUN_ROOT="$RUN_ROOT" \
    "$BUILD_JOB_FILE"
)"
...

{
  ...
  printf 'FF3D_TOOLCHAIN_LOCK_SHA256=%q\n' "$TOOLCHAIN_LOCK_SHA256"
