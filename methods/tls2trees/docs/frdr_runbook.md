...
[`frdr_results.md`](frdr_results.md).
...
- Patched instance script: `methods/tls2trees/scripts/runtime/patches/instance_patched.py`
- Config: `methods/tls2trees/configs/frdr_benchmark.yml`
...
test -f methods/tls2trees/scripts/runtime/patches/instance_patched.py
python methods/tls2trees/scripts/runtime/patches/instance_patched.py --help
```
...
```bash
cd ~/scratch/tree-seg-benchmark
sbatch methods/tls2trees/slurm/frdr/inspect_frdr_inventory.sbatch
squeue -u "$USER"
```
...
CONVERT_PILOT_JOB=$(sbatch --parsable --array=0-0 \
  methods/tls2trees/slurm/frdr/convert_frdr_to_tls2trees_array.sbatch)
...
python methods/tls2trees/scripts/runtime/run_tls2trees_instance_for_plot.py \
  --config methods/tls2trees/configs/frdr_benchmark.yml \
...
RUN_PILOT_JOB=$(sbatch --parsable --array=0-0 \
  methods/tls2trees/slurm/frdr/run_tls2trees_frdr_array.sbatch)
echo "$RUN_PILOT_JOB"
...
python methods/tls2trees/scripts/runtime/summarise_tls2trees_outputs.py \
  --plot-name LPine_plot1 \
...
SUMMARY_JOB=$(sbatch --parsable --dependency=afterok:"$RUN_JOB" \
  methods/tls2trees/slurm/frdr/summarise_tls2trees_frdr_outputs.sbatch)
...
```bash
sbatch --array=2-2 --mem=96G methods/tls2trees/slurm/frdr/run_tls2trees_frdr_array.sbatch
```
...
```bash
sbatch methods/tls2trees/slurm/frdr/summarise_tls2trees_frdr_outputs.sbatch
```
