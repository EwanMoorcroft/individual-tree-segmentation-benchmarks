...
FINAL_EVALUATION_PLAN_JSON="${TLS2TREES_FINAL_EVALUATION_PLAN_JSON:?evaluation state has no frozen plan}"
FINAL_EVALUATION_PLAN_SHA256="${TLS2TREES_FINAL_EVALUATION_PLAN_SHA256:?evaluation state has no frozen plan SHA-256}"
FINAL_EVALUATION_EVALUATOR="${TLS2TREES_FINAL_EVALUATION_EVALUATOR:?evaluation state has no frozen evaluator path}"
FINAL_EVALUATION_EVALUATOR_SHA256="${TLS2TREES_FINAL_EVALUATION_EVALUATOR_SHA256:?evaluation state has no frozen evaluator SHA-256}"
...
cd "$PROJECT_ROOT"
test -d .git
RECOVERY_CONFIRMED="${TLS2TREES_FINALIZE_RESULTS_RECOVERY_CONFIRMED:-0}"
# shellcheck source=methods/tls2trees/slurm/for_instance/held_out_finalisation_worktree_gate.sh
source methods/tls2trees/slurm/for_instance/held_out_finalisation_worktree_gate.sh
tls2trees_validate_held_out_finalisation_worktree \
  "$PROJECT_ROOT" "$RECOVERY_CONFIRMED"
...
test -s "$FINAL_EVALUATION_PLAN_JSON"
test "$(sha256sum "$FINAL_EVALUATION_PLAN_JSON" | awk '{print $1}')" = \
  "$FINAL_EVALUATION_PLAN_SHA256"
test -f "$FINAL_EVALUATION_EVALUATOR"
...
JOB_ID=$(sbatch --parsable \
  --export="ALL,TLS2TREES_FINALIZE_RESULTS_CONFIRMED=1,TLS2TREES_FINALIZE_RESULTS_RECOVERY_CONFIRMED=$RECOVERY_CONFIRMED,TLS2TREES_PROJECT_ROOT=$PROJECT_ROOT,TLS2TREES_FINALIZE_BENCHMARK_COMMIT=$BENCHMARK_COMMIT,TLS2TREES_FINALIZE_RECEIPT_JSON=$RECEIPT_JSON,TLS2TREES_TEST_TREEBENCH_ENV=$TLS2TREES_TEST_TREEBENCH_ENV,TLS2TREES_TEST_SUMMARY_JSON=$FINAL_SUMMARY_JSON,TLS2TREES_TEST_MANIFEST_JSON=$TLS2TREES_TEST_MANIFEST_JSON,TLS2TREES_TEST_FINAL_SELECTION_JSON=$TLS2TREES_TEST_FINAL_SELECTION_JSON,TLS2TREES_TEST_FINAL_SELECTION_SHA256=$TLS2TREES_TEST_FINAL_SELECTION_SHA256,TLS2TREES_TEST_RUN_ID=$TLS2TREES_TEST_RUN_ID,TLS2TREES_FINAL_EVALUATION_BENCHMARK_COMMIT=$FINAL_EVALUATION_BENCHMARK_COMMIT,TLS2TREES_FINAL_EVALUATION_RUN_ID=$FINAL_EVALUATION_RUN_ID,TLS2TREES_FINAL_EVALUATION_PLAN_JSON=$FINAL_EVALUATION_PLAN_JSON,TLS2TREES_FINAL_EVALUATION_PLAN_SHA256=$FINAL_EVALUATION_PLAN_SHA256,TLS2TREES_FINAL_EVALUATION_EVALUATOR=$FINAL_EVALUATION_EVALUATOR,TLS2TREES_FINAL_EVALUATION_EVALUATOR_SHA256=$FINAL_EVALUATION_EVALUATOR_SHA256" \
...
FINALIZE_STATE="$HOME/fastscratch/tls2trees_for_instance_test_states/${TLS2TREES_TEST_RUN_ID}_finalisation.env"
{
  ...
  printf 'TLS2TREES_FINALIZE_RESULTS_RECOVERY_CONFIRMED=%q\n' \
    "$RECOVERY_CONFIRMED"
} > "$FINALIZE_STATE"
printf '%s\n' "$FINALIZE_STATE" > \
