...
CHECKPOINT_OVERRIDE="${TREELEARN_FINETUNE_CHECKPOINT_OVERRIDE:-}"
VALIDATION_TAG="${TREELEARN_FINETUNE_VALIDATION_TAG:-checkpoint}"
PRIOR_JOB="${TREELEARN_FINETUNE_VALIDATION_AFTEROK_JOB:-}"
...
if [[ -n "$CHECKPOINT_OVERRIDE" ]]; then
  TREELEARN_FINETUNE_CHECKPOINT="$CHECKPOINT_OVERRIDE"
fi
[[ "$VALIDATION_TAG" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || {
  echo "Unsafe TREELEARN_FINETUNE_VALIDATION_TAG." >&2; exit 2;
}
if [[ -n "$PRIOR_JOB" && ! "$PRIOR_JOB" =~ ^[0-9]+$ ]]; then
  echo "TREELEARN_FINETUNE_VALIDATION_AFTEROK_JOB must be a job ID." >&2; exit 2
fi
cd "$PROJECT_ROOT"
...
TRAINING_RUN_ID="$TREELEARN_FINETUNE_RUN_ID"
STAMP="${TREELEARN_FINETUNE_VALIDATION_STAMP:-$(date -u +%Y%m%d_%H%M%S)}"
VALIDATION_RUN_ID="${TRAINING_RUN_ID}_${VALIDATION_TAG}_validation_$STAMP"
METADATA_ROOT="$PROJECT_ROOT/results/metadata/treelearn_for_instance/development_runs/$VALIDATION_RUN_ID"
TABLE_ROOT="$PROJECT_ROOT/results/tables/treelearn_for_instance/development_runs/$VALIDATION_RUN_ID"
STATE_FILE="$HOME/fastscratch/treelearn_finetune_validation_${VALIDATION_RUN_ID}.env"
test ! -e "$METADATA_ROOT"; test ! -e "$TABLE_ROOT"; test ! -e "$STATE_FILE"
EXPORTS="ALL,TREELEARN_FINETUNE_VALIDATION_CONFIRMED=1,TREELEARN_EXPECTED_BENCHMARK_COMMIT=$COMMIT,TREELEARN_ENV=$TREELEARN_ENV,TREELEARN_REPO=$TREELEARN_REPO,TREELEARN_DATASET_ROOT=$TREELEARN_DATASET_ROOT,TREELEARN_FINETUNE_RUN_ID=$VALIDATION_RUN_ID,TREELEARN_FINETUNE_CHECKPOINT=$TREELEARN_FINETUNE_CHECKPOINT,TREELEARN_FINETUNE_CHECKPOINT_SHA256=$SHA,TREELEARN_FINETUNE_FREEZE=$FREEZE,TREELEARN_DEV_MANIFEST_JSON=$DEV_MANIFEST,TREELEARN_FINETUNE_METADATA_ROOT=$METADATA_ROOT,TREELEARN_FINETUNE_TABLE_ROOT=$TABLE_ROOT"
DEPENDENCY_ARGS=()
if [[ -n "$PRIOR_JOB" ]]; then
  DEPENDENCY_ARGS=(--dependency="afterok:$PRIOR_JOB" --kill-on-invalid-dep=yes)
fi
ARRAY=$(sbatch --parsable "${DEPENDENCY_ARGS[@]}" --array=0,3,7,8,20%2 --export="$EXPORTS" methods/treelearn/slurm/run_for_instance_finetune_validation.sbatch)
...
{
  printf 'TREELEARN_FINETUNE_TRAINING_RUN_ID=%q\n' "$TRAINING_RUN_ID"
  printf 'TREELEARN_FINETUNE_VALIDATION_RUN_ID=%q\n' "$VALIDATION_RUN_ID"
  printf 'TREELEARN_FINETUNE_VALIDATION_TAG=%q\n' "$VALIDATION_TAG"
  printf 'TREELEARN_FINETUNE_VALIDATION_CHECKPOINT=%q\n' "$TREELEARN_FINETUNE_CHECKPOINT"
  printf 'TREELEARN_FINETUNE_VALIDATION_CHECKPOINT_SHA256=%q\n' "$SHA"
