...
ATTEMPT="${TREELEARN_PRETRAINED_TEST_RECOVERY_ATTEMPT:-1}"
if [[ "$ATTEMPT" != "1" && "$ATTEMPT" != "2" ]]; then
  echo "Recovery attempt must be 1 or 2." >&2
  exit 2
fi
...
if [[ "$ATTEMPT" = "1" ]]; then
  ...
  PRIOR_RECOVERY_MANIFEST="$TREELEARN_TEST_METADATA_ROOT/task_8_execution_recovery.json"
  test -f "$PRIOR_RECOVERY_MANIFEST"
  RECOVERY_MANIFEST="$TREELEARN_TEST_METADATA_ROOT/task_8_execution_recovery_attempt_2.json"
  RECOVERY_ARCHIVE="$HOME/fastscratch/treelearn_pretrained_test_recovery_archive/${TREELEARN_TEST_RUN_ID}_attempt_2"
  RECOVERY_GUARD="$HOME/fastscratch/treelearn_pretrained_test_recovery_once/${TREELEARN_TEST_RUN_ID}_attempt_2"
fi
RECOVERY_STATE="$HOME/fastscratch/treelearn_pretrained_test_recovery_${TREELEARN_TEST_RUN_ID}_attempt_${ATTEMPT}.env"
